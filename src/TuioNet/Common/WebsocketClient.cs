        ...
        Task.Run(async () =>
        {
            while (_socket is not { State: WebSocketState.Open })
            {
                ...
                using (_socket = new ClientWebSocket())
                {
                    ...
                    {
                        _logger.LogError("[WebsocketClient] Could not connect to {BoxUrl} -> {Message}", _boxUrl, exception.Message);
    ...
    private async Task ReceiveMessages()
    {
        while (_socket.State == WebSocketState.Open)
        {
            ...
            while (true)
            {
                ...
                {
                    _logger.LogError("[WebsocketClient] Could not receive message from {BoxUrl} -> {Message}", _boxUrl, exception.Message);
                    break;
                }
            }
